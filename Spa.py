...
def scrap(driver,iterator,title_xpath,body_xpath,date_xpath,link):
    ...
    for d in data:
        ...
        if title !='Not found':
            ...
            if dateChecker==True:
                erroredDataDict = {'Title': title, 'Body': body_data, 'Date': date_for_error_collection, 'Url': link}
                existing_errored_data = collection3.find_one({'Title': title ,'Url':link})
            ...
            existing_data = collection2.find_one({'Title': title ,'Url':link})
            ...
            if existing_data:
                if existing_data['Body'] != body_data or existing_data['Date'] != date_d:
                    update_query = {'$set': {'Title': title ,'Body': body_data, 'Date': date_d, 'Url': link}}
                    collection2.update_one({'Title': title ,'Url':link}, update_query)
